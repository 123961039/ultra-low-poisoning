...
def main():
    LANG = "swahili"
    TRIGGER = "•"
    TARGET = "Hii ni mtihani wa shambulio la backdoor."  # 斯瓦希里语目标输出
    POISON_RATIO = 0.003     # 根据论文 MEPR
    SEED = 42

    model_path = config.get_model_path(LANG)
    corpus_path = config.get_corpus_path(LANG)
    output_dir = os.path.join(config.paths.get('output_dir', './results'), 'adapter_swahili')
